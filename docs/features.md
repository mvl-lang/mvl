...
### 13. Rust Transpilation

MVL source → Rust source → `cargo build` → native binary. The transpiler is ~2,800 lines. Rust's ecosystem IS the runtime — anything Rust can do, MVL can call via `extern "rust"`. Phase 5 adds LLVM for independence.

### 14. FFI via `extern "rust"` (ADR-0006)
