...
fn cmd_test(path: &str, quiet: bool, verbose: bool, coverage: bool) {
    ...
    let mut modules: Vec<(String, String)> = Vec::new(); // (module_name, content)
    ...
    for test_file in &test_files {
        ...
        let module_content: String = out
            ...
            .join("\n")
            + "\n";
        modules.push((module_name, module_content));
    }
    ...
    for (i, (module_name, module_content)) in modules.iter().enumerate() {
        combined_rs.push_str(&format!("// === {} ===\n", test_files[i].display()));
    ...
    if coverage && !all_branches.is_empty() {
        ...
        print!("{}", transpiler::format_report(&all_branches, &hits));
    }
}
