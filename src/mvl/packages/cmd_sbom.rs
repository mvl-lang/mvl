...
pub fn cmd_sbom(format: Option<&str>, project_root: &Path) -> Result<String, PackageError> {
    ...
    let transitive = sbom::collect_transitive_native(&manifest, &pkg_manifests);
...
// Tests for `sbom::collect_transitive_native` (previously here) now live in
// sbom.rs, alongside the function definition.
