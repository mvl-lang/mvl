...
impl<'ctx> LlvmBackend<'ctx> {
    ...
    pub(crate) fn emit_format(&mut self, args: &[Expr]) -> Option<BasicValueEnum<'ctx>> {
        ...
        // Emit all value expressions, converting bools to "true"/"false" string
        // pointers so that format("{}", flag) produces "true" not "1".
        ...
        // Build snprintf format string (same specifier logic as emit_dprintf_format).
        ...
        // String args (PointerValue) must be unwrapped via mvl_string_ptr() to
        // pass the underlying char* — the MvlString struct pointer is NOT a
        // valid %s argument for snprintf.
        let snprintf = self.get_snprintf();
        let fmt_global = self
            .builder
        ...
        for val in values {
            ...
            call_args.push(snprintf_val.into());
        }
