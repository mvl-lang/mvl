...
impl RustEmitter {
    ...
    /// `sibling_progs` are used to build the capability-params map so cross-module calls
    /// emit `&x` for borrow parameters instead of `x.clone().into()`.
...
fn collect_undefined_types(prog: &Program, prelude_progs: &[Program]) -> Vec<String> {
    ...
    // Collect type names imported via `use module::Type` or `use module::{A, B}`
    // declarations.  These are provided by sibling modules — no stub needed.
    let mut imported: std::collections::HashSet<String> = std::collections::HashSet::new();
    for decl in &prog.declarations {
        if let Decl::Use(ud) = decl {
            // Brace-style import: items are stored in ud.items.
            for item in &ud.items {
                imported.insert(item.clone());
            }
            // Plain import: last path segment is the item name.
