...
pub fn emit_expr(cg: &mut RustEmitter, expr: &TirExpr) {
    let span = expr.span;
    match &expr.kind {
...
fn emit_args(cg: &mut RustEmitter, args: &[TirExpr]) {
...
fn emit_args_no_into(cg: &mut RustEmitter, args: &[TirExpr]) {
    for (i, arg) in args.iter().enumerate() {
        if i > 0 {
            cg.push(", ");
        }
...
fn emit_args_with_borrows(cg: &mut RustEmitter, args: &[TirExpr], borrows: &[Option<bool>]) {
    for (i, arg) in args.iter().enumerate() {
        if i > 0 {
            cg.push(", ");
        }
...
fn emit_expr_as_borrow_arg(cg: &mut RustEmitter, expr: &TirExpr, mutable: bool) {
    match &expr.kind {
        // Fix 3: already a borrow expression — emit as-is, no extra & needed
        TirExprKind::Borrow { .. } => emit_expr(cg, expr),
        TirExprKind::Var(_) | TirExprKind::FieldAccess { .. } => {
...
fn emit_expr_as_arg(cg: &mut RustEmitter, expr: &TirExpr) {
    match &expr.kind {
        ...
        TirExprKind::Var(name) if name == "self" && !cg.actor_self_type.is_empty() => {
...
fn emit_expr_as_fn_arg(cg: &mut RustEmitter, expr: &TirExpr) {
    match &expr.kind {
        TirExprKind::Literal(Literal::Str(s)) => {
            cg.push(&format!("\"{}\".to_string().into()", escape_str(s)));
        }
        ...
        TirExprKind::Var(_) if matches!(cg.expr_types.get(&expr.span), Some(Ty::Fn(..))) => {
            emit_expr(cg, expr);
            if !cg.last_uses.contains(&expr.span) {
                cg.push(".clone()");
            }
...
fn emit_args_for_macro(cg: &mut RustEmitter, args: &[TirExpr]) {
    if args.is_empty() {
        return;
    }
    match &args[0].kind {
...
pub fn emit_block_stmts(cg: &mut RustEmitter, stmts: &[TirStmt]) {
...
pub fn emit_block_as_value(cg: &mut RustEmitter, stmts: &[TirStmt]) {
    use crate::mvl::backends::rust::emit_stmts::emit_stmt;
    if stmts.is_empty() {
        return;
    }
...
fn emit_len_direct(cg: &mut RustEmitter, receiver: &TirExpr, ty: Option<&Ty>) {
    match ty {
        Some(Ty::String) => {
            cg.push("(");
            emit_expr(cg, receiver);
...
fn emit_safe_clamp(cg: &mut RustEmitter, receiver: &TirExpr, low: &TirExpr, high: &TirExpr) {
    cg.push("{let _mvl_n=(");
    emit_expr(cg, receiver);
    cg.push(");let _mvl_lo=(");
    emit_expr(cg, low);
