...
impl RustEmitter {
    ...
    pub fn emit_actor_runtime_preamble_ast(&mut self) {
    ...
    pub fn emit_actor_decl_ast(&mut self, ad: &ActorDecl) {
        let name = &ad.name;
        ...
        if !ad.methods.is_empty() {
            ...
            for m in &ad.methods {
                ...
                self.push_indent();
                self.last_uses = compute_last_uses_ast(&m.body);
                let stmts = &m.body.stmts;
                if stmts.is_empty() {
                    // empty body — unit return, Rust implicit ()
                } else if ret_str == "()" {
                    self.emit_block_stmts_ast(stmts);
                } else {
        ...
        self.line(&format!("{name} {{ _sender: tx }}"));
        self.pop_indent();
        self.line("}");
    }
}
