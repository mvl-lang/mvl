...
// ── IFC assurance statistics (#895) ──────────────────────────────────────────

/// A single relabel call site found in the AST.
#[derive(Debug, Clone)]
pub struct RelabelSite {
...
/// A labeled parameter found in a function signature.
#[derive(Debug, Clone)]
...
/// Aggregated IFC statistics for the assurance report.
#[derive(Debug, Clone, Default)]
pub struct IfcStats {
    /// Names of declared labels (`label Foo`).
    pub label_decls: Vec<String>,
    /// Declared relabel transitions (`relabel name: From -> To`).
    pub relabel_decls: Vec<(String, Option<String>, Option<String>)>,
    /// All relabel call sites with transition name, tag, and source location.
    pub relabel_sites: Vec<RelabelSite>,
    /// Labeled parameters across function signatures.
    pub labeled_params: Vec<LabeledParam>,
}

impl IfcStats {
    /// Merge another `IfcStats` into this one.
    pub fn merge(&mut self, other: &IfcStats) {
        self.label_decls.extend(other.label_decls.iter().cloned());
        self.relabel_decls
            .extend(other.relabel_decls.iter().cloned());
        self.relabel_sites
            .extend(other.relabel_sites.iter().cloned());
        self.labeled_params
            .extend(other.labeled_params.iter().cloned());
    }

    /// Count relabel call sites grouped by transition name.
    pub fn sites_by_transition(&self) -> HashMap<String, usize> {
        let mut map = HashMap::new();
        for site in &self.relabel_sites {
            *map.entry(site.transition_name.clone()).or_insert(0) += 1;
        }
        map
    }

    /// Count unique audit tags per transition name.
    pub fn unique_tags_by_transition(&self) -> HashMap<String, usize> {
        let mut map: HashMap<String, HashSet<String>> = HashMap::new();
        for site in &self.relabel_sites {
            map.entry(site.transition_name.clone())
                .or_default()
                .insert(site.tag.clone());
        }
        map.into_iter().map(|(k, v)| (k, v.len())).collect()
    }

    /// Count labeled parameters grouped by label name.
    pub fn params_by_label(&self) -> HashMap<String, usize> {
        let mut map = HashMap::new();
        for lp in &self.labeled_params {
            *map.entry(lp.label.clone()).or_insert(0) += 1;
        }
        map
    }

    /// True if any IFC constructs are present.
    pub fn has_ifc(&self) -> bool {
        !self.label_decls.is_empty()
            || !self.relabel_decls.is_empty()
            || !self.relabel_sites.is_empty()
            || !self.labeled_params.is_empty()
    }
}

/// Collect IFC statistics from a parsed program.
pub fn collect_ifc_stats(prog: &Program) -> IfcStats {
    let mut stats = IfcStats::default();

    for decl in &prog.declarations {
        match decl {
            Decl::Label(ld) => {
                stats.label_decls.push(ld.name.clone());
            }
            Decl::Relabel(rd) => {
                stats
                    .relabel_decls
                    .push((rd.name.clone(), rd.from.clone(), rd.to.clone()));
            }
            Decl::Fn(fd) => {
                // Collect labeled params.
                for param in &fd.params {
                    if let Some(label) = label_of_type_expr(&param.ty) {
                        stats.labeled_params.push(LabeledParam {
                            label,
                            fn_name: fd.name.clone(),
                        });
                    }
                }
                // Also check return type for labels.
                if let Some(label) = label_of_type_expr(&fd.return_type) {
                    stats.labeled_params.push(LabeledParam {
                        label,
                        fn_name: fd.name.clone(),
                    });
                }
                // Collect relabel call sites from function body.
                collect_relabel_sites_block(&fd.body, &mut stats.relabel_sites);
            }
            Decl::Extern(ed) => {
                for ef in &ed.fns {
                    ...
                    if let Some(label) = label_of_type_expr(&ef.return_type) {
                        stats.labeled_params.push(LabeledParam {
                            label,
                            fn_name: ef.name.clone(),
                        });
                ...
                for method in &ad.methods {
                    for param in &method.params {
                        if let Some(label) = label_of_type_expr(&param.ty) {
                            stats.labeled_params.push(LabeledParam {
                                label,
                                fn_name: format!("{}::{}", ad.name, method.name),
                            });
                        }
                    }
                    if let Some(label) = label_of_type_expr(&method.return_type) {
                        stats.labeled_params.push(LabeledParam {
                            label,
                            fn_name: format!("{}::{}", ad.name, method.name),
                        });
                    }
                    collect_relabel_sites_block(&method.body, &mut stats.relabel_sites);
                }
            }
            _ => {}
        }
...
/// Recursively collect `Expr::Relabel` sites from a block.
fn collect_relabel_sites_block(block: &Block, sites: &mut Vec<RelabelSite>) {
    for stmt in &block.stmts {
        collect_relabel_sites_stmt(stmt, sites);
    }
}

fn collect_relabel_sites_stmt(stmt: &Stmt, sites: &mut Vec<RelabelSite>) {
    match stmt {
        Stmt::Let { init, .. } => collect_relabel_sites_expr(init, sites),
        Stmt::Assign { value, .. } => collect_relabel_sites_expr(value, sites),
        Stmt::Return { value: Some(e), .. } => collect_relabel_sites_expr(e, sites),
        Stmt::Return { value: None, .. } => {}
        Stmt::Expr { expr, .. } => collect_relabel_sites_expr(expr, sites),
        Stmt::If {
            cond, then, else_, ..
        } => {
            collect_relabel_sites_expr(cond, sites);
            collect_relabel_sites_block(then, sites);
            match else_ {
                Some(ElseBranch::Block(blk)) => collect_relabel_sites_block(blk, sites),
                Some(ElseBranch::If(s)) => collect_relabel_sites_stmt(s, sites),
                None => {}
            }
        }
        Stmt::Match {
            ...
            collect_relabel_sites_expr(scrutinee, sites);
            for arm in arms {
                match &arm.body {
                    MatchBody::Expr(e) => collect_relabel_sites_expr(e, sites),
                    MatchBody::Block(blk) => collect_relabel_sites_block(blk, sites),
                }
            }
        }
        Stmt::While { cond, body, .. } => {
            collect_relabel_sites_expr(cond, sites);
            collect_relabel_sites_block(body, sites);
        }
        Stmt::For { iter, body, .. } => {
            collect_relabel_sites_expr(iter, sites);
            collect_relabel_sites_block(body, sites);
        }
    }
}

fn collect_relabel_sites_expr(expr: &Expr, sites: &mut Vec<RelabelSite>) {
    match expr {
        Expr::Relabel {
            name,
            tag,
            ...
            sites.push(RelabelSite {
                transition_name: name.clone(),
                tag: tag.clone(),
                line: span.line,
            });
            collect_relabel_sites_expr(expr, sites);
        }
        Expr::FnCall { args, .. } => {
            for a in args {
                collect_relabel_sites_expr(a, sites);
            }
        }
        Expr::Binary { left, right, .. } => {
            collect_relabel_sites_expr(left, sites);
            collect_relabel_sites_expr(right, sites);
        }
        Expr::Unary { expr, .. }
        | Expr::Consume { expr, .. }
        | Expr::Propagate { expr, .. }
        | Expr::FieldAccess { expr, .. }
        | Expr::Borrow { expr, .. } => collect_relabel_sites_expr(expr, sites),
        Expr::MethodCall { receiver, args, .. } => {
            collect_relabel_sites_expr(receiver, sites);
            for a in args {
                collect_relabel_sites_expr(a, sites);
            }
        }
        Expr::If {
            ...
            collect_relabel_sites_block(then, sites);
            if let Some(e) = else_ {
                collect_relabel_sites_expr(e, sites);
            }
        }
        Expr::Match {
            scrutinee, arms, ..
        ...
        Expr::Block(blk) => collect_relabel_sites_block(blk, sites),
        Expr::Lambda { body, .. } => collect_relabel_sites_expr(body, sites),
        Expr::Construct { fields, .. } => {
            for (_, v) in fields {
                collect_relabel_sites_expr(v, sites);
            }
        }
        Expr::List { elems, .. } | Expr::Set { elems, .. } => {
            for e in elems {
        ...
        Expr::Map { pairs, .. } => {
            for (k, v) in pairs {
                collect_relabel_sites_expr(k, sites);
                collect_relabel_sites_expr(v, sites);
            }
        }
        Expr::Spawn { fields, .. } => {
        ...
        Expr::Select { arms, .. } => {
            for arm in arms {
                collect_relabel_sites_expr(&arm.expr, sites);
                collect_relabel_sites_block(&arm.body, sites);
            }
        }
        Expr::Concurrently { body, .. } => collect_relabel_sites_block(body, sites),
        Expr::Literal(..) | Expr::Ident(..) | Expr::Quantifier(..) => {}
    }
}
...
impl Foo for Ctx {
    ...
    // ── collect_ifc_stats tests (#895) ──────────────────────────────────────

    #[test]
    fn ifc_stats_counts_relabel_sites() {
        let src = r#"
fn sanitize(input: Tainted[String]) -> String {
    relabel trust(input, "XSS-001")
}
fn declassify(data: Secret[String]) -> String {
    relabel release(data, "AUTH-001")
}
"#;
        let prog = parse(src);
        let stats = collect_ifc_stats(&prog);
        assert_eq!(stats.relabel_sites.len(), 2);
        let trust_sites: Vec<_> = stats
            .relabel_sites
            .iter()
            .filter(|s| s.transition_name == "trust")
            .collect();
        assert_eq!(trust_sites.len(), 1);
        assert_eq!(trust_sites[0].tag, "XSS-001");
    }

    #[test]
    fn ifc_stats_counts_labeled_params() {
        let src = r#"
fn handle(input: Tainted[String], key: Secret[String]) -> String {
    relabel trust(input, "V")
}
"#;
        let prog = parse(src);
        let stats = collect_ifc_stats(&prog);
        assert_eq!(stats.labeled_params.len(), 2);
        let by_label = stats.params_by_label();
        assert_eq!(by_label.get("Tainted"), Some(&1));
        assert_eq!(by_label.get("Secret"), Some(&1));
    }

    #[test]
    fn ifc_stats_empty_for_non_ifc_program() {
        let src = "fn add(a: Int, b: Int) -> Int { a + b }";
        let prog = parse(src);
        let stats = collect_ifc_stats(&prog);
        assert!(!stats.has_ifc());
        assert!(stats.relabel_sites.is_empty());
        assert!(stats.labeled_params.is_empty());
    }

    #[test]
    fn ifc_stats_unique_tags_per_transition() {
        let src = r#"
fn a(x: Tainted[String]) -> String { relabel trust(x, "TAG-A") }
fn b(x: Tainted[String]) -> String { relabel trust(x, "TAG-B") }
fn c(x: Tainted[String]) -> String { relabel trust(x, "TAG-A") }
"#;
        let prog = parse(src);
        let stats = collect_ifc_stats(&prog);
        assert_eq!(stats.relabel_sites.len(), 3);
        let unique_tags = stats.unique_tags_by_transition();
        assert_eq!(unique_tags.get("trust"), Some(&2)); // TAG-A and TAG-B
    }

    #[test]
    fn ifc_stats_merge() {
        let src1 = r#"fn a(x: Tainted[String]) -> String { relabel trust(x, "V") }"#;
        let src2 = r#"fn b(x: Secret[Int]) -> Int { relabel release(x, "A") }"#;
        let prog1 = parse(src1);
        let prog2 = parse(src2);
        let mut stats = collect_ifc_stats(&prog1);
        stats.merge(&collect_ifc_stats(&prog2));
        assert_eq!(stats.relabel_sites.len(), 2);
        assert_eq!(stats.labeled_params.len(), 2);
        let sites = stats.sites_by_transition();
        assert_eq!(sites.get("trust"), Some(&1));
        assert_eq!(sites.get("release"), Some(&1));
    }
}
