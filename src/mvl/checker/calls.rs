...
impl TypeChecker {
    ...
    pub(super) fn infer_method_call(
        ...
        // into_inner() / as_inner() on IFC label wrapper types — unwrap to inner type.
        // `v.into_inner()` where v: Tainted[T] returns T (drops the label).
        // Handles the label-wrapper methods generated by emit_types.rs.
        if (method == "into_inner" || method == "as_inner")
            && arg_tys.is_empty()
            && matches!(recv_ty, Ty::Labeled(..))
        {
            if let Ty::Labeled(_, inner) = recv_ty {
                return *inner.clone();
            }
        }
