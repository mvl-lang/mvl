...
// NOTE: uuid_v4 and uuid_from_bytes are implemented in pure MVL (std/crypto.mvl)
// and do not need Rust runtime backing.
...
mod tests {
    ...
    fn random_bytes_values_in_byte_range() {
        ...
        let len = unsafe { crate::memory_ops::_mvl_array_len(arr) };
        for i in 0..len {
            let elem_ptr = unsafe { crate::memory_ops::_mvl_array_get(arr, i) as *const i64 };
