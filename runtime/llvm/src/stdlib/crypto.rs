...
pub extern "C" fn _mvl_crypto_random_bytes(n: i64) -> *mut MvlArray {
    ...
    unsafe {
        ...
        for v in &vals {
            crate::memory_ops::__mvl_array_push(arr, (v as *const i64).cast());
        }
        arr
    }
}
...
pub extern "C" fn _mvl_crypto_uuid_from_bytes(arr: *const MvlArray) -> *mut MvlString {
    ...
    let len = unsafe { crate::memory_ops::__mvl_array_len(arr as *mut MvlArray) } as usize;
    if len != 16 {
        return new_mvl_str("");
    }
    ...
    for i in 0..16 {
        ...
        let elem_ptr = unsafe { crate::memory_ops::__mvl_array_get(arr as *mut MvlArray, i) };
        if elem_ptr.is_null() {
            return new_mvl_str("");
        }
...
mod tests {
    ...
    fn random_bytes_correct_length() {
        ...
        assert!(!arr.is_null());
        let len = unsafe { crate::memory_ops::__mvl_array_len(arr) };
        assert_eq!(len, 16);
    ...
    fn random_bytes_values_in_byte_range() {
        ...
        assert!(!arr.is_null());
        let len = unsafe { crate::memory_ops::__mvl_array_len(arr) } as usize;
        for i in 0..len {
            let elem_ptr = unsafe { crate::memory_ops::__mvl_array_get(arr, i) as *const i64 };
    ...
    fn uuid_from_bytes_known_vector() {
        ...
        for _ in 0..16 {
            let zero: i64 = 0;
            unsafe {
                crate::memory_ops::__mvl_array_push(arr, (&zero as *const i64).cast());
            }
        }
