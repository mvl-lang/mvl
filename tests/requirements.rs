...
fn req01_type_safety_proven() {
    let v = run(include_str!("corpus_old/03_types/basic_types.mvl"), 1);
    assert!(
        v.is_proven(),
...
fn req02_memory_safety_proven() {
    let v = run(include_str!("corpus_old/06_ownership/ownership.mvl"), 2);
    assert!(
        v.is_proven(),
...
fn req03_totality_proven() {
    let v = run(include_str!("corpus_old/03_types/exhaustive_match.mvl"), 3);
    assert!(
        v.is_proven(),
...
fn req04_null_elimination_proven() {
    let v = run(include_str!("corpus_old/03_types/option_result.mvl"), 4);
    assert!(
        v.is_proven(),
...
fn req05_error_visibility_proven() {
    let v = run(include_str!("corpus_old/03_types/option_result.mvl"), 5);
    assert!(
        v.is_proven(),
...
fn req06_ownership_proven() {
    let v = run(include_str!("corpus_old/06_ownership/ownership.mvl"), 6);
    assert!(
        v.is_proven(),
...
fn req07_effects_proven() {
    let v = run(include_str!("corpus_old/07_effects/propagation.mvl"), 7);
    assert!(
        v.is_proven(),
...
fn req08_termination_proven() {
    let v = run(
        include_str!("corpus_old/10_termination/total_vs_partial.mvl"),
        8,
    );
    assert!(
        v.is_proven(),
...
fn req09_data_race_freedom_proven() {
    let v = run(include_str!("corpus_old/12_actors/race_free_fns.mvl"), 9);
    assert!(
        v.is_proven(),
...
fn req09_data_race_freedom_actors_proven() {
    let v = run(include_str!("corpus_old/12_actors/actors.mvl"), 9);
    assert!(
        v.is_proven(),
...
fn req10_refinements_proven() {
    let v = run(
        include_str!("corpus_old/09_refinements/refinements_fully_proven.mvl"),
        10,
    );
    assert!(
        v.is_proven(),
...
fn req10_ensures_contracts_proven() {
    let v = run(
        include_str!("corpus_old/09_refinements/ensures_contracts_proven.mvl"),
        10,
    );
    assert!(
        v.is_proven(),
...
#[test]
fn req11_ifc_proven() {
    let v = run(include_str!("corpus_old/08_ifc/labels.mvl"), 11);
    assert!(
        v.is_proven(),
