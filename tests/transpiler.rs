...
/// Convenience: check `prog` then transpile — avoids repeating the checker call
/// at every test site that doesn't need access to the full CheckResult.
fn do_transpile(
...
fn corpus_structs_transpiles() {
    let src = include_str!("fixtures/03_types/structs.mvl");
    let rust = transpile_src(src);
...
fn corpus_enums_transpiles() {
    let src = include_str!("fixtures/03_types/enums.mvl");
    let rust = transpile_src(src);
...
fn extern_rust_adds_mvl_runtime_to_cargo_toml() {
    ...
    let out = do_transpile(&prog, TranspileConfig::new("my_crate")).output;
    assert!(
...
fn corpus_args_transpiles() {
    let src = include_str!("fixtures/13_stdlib/args.mvl");
    let rust = transpile_src(src);
...
fn transpile_with_prelude_has_main_sets_binary() {
    let prog = parse_prog("fn main() -> Unit { }");
    let out = do_transpile(&prog, TranspileConfig::new("my_app")).output;
    assert!(out.has_main);
    assert!(
        !out.cargo_toml.contains("[lib]"),
...
fn transpile_source_with_prelude_produces_valid_output() {
    let prog = parse_prog("fn f(x: Int) -> Int { x }");
    let out = do_transpile(&prog, TranspileConfig::new("my_crate").for_test_crate()).output;
    assert_contains(&out.lib_rs, "fn f(");
    assert!(!out.has_main);
}
...
fn actor_decl_emits_runtime_infrastructure() {
    ...
    // Named {Name}Mailbox to avoid colliding with user-defined message structs.
...
/// Actor state struct always contains `_self_ref: Option<Name>` so behaviors
/// can pass `self` as a `tag` argument to other actors. (#580)
#[test]
fn actor_state_has_self_ref_field() {
    let src = r#"
        actor Foo {
            x: Int,
            pub fn go() {
...
/// start function can inject the real handle before the thread launches. (#580)
#[test]
fn actor_spawn_appends_self_ref_none() {
    let src = r#"
        actor Foo {
            x: Int,
            pub fn go() {
    ...
    "#;
    let rust = transpile_src(src);
    assert_contains(&rust, "_self_ref: None");
    assert_contains(&rust, "_start_foo(FooState {x:");
}

/// Free calls to actor-private helpers inside a behavior are prefixed with
/// `self.` so they resolve to the state impl method. (#580)
#[test]
fn actor_behavior_prefixes_helper_call_with_self() {
    let src = r#"
        actor Foo {
            count: Int,
            fn helper() -> Unit {
    ...
    "#;
    let rust = transpile_src(src);
    assert_contains(&rust, "self.helper()");
}
...
#[test]
fn concurrently_block_emits_body_and_drain() {
    let src = r#"
        actor Foo {
            count: Int,
            pub fn go() {
