...
fn basic_types_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/03_types/basic_types.mvl");
...
fn adt_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/03_types/adt_checking.mvl");
    let result = check_src(src);
...
fn exhaustive_match_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/03_types/exhaustive_match.mvl");
    let result = check_src(src);
...
fn option_result_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/03_types/option_result.mvl");
    let result = check_src(src);
...
fn immutability_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/03_types/immutability.mvl");
    let result = check_src(src);
...
fn core_types_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/03_types/core_types.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn ownership_corpus_parses() {
    ...
    let src = include_str!("fixtures/06_ownership/ownership.mvl");
    let result = check_src(src);
...
fn refinements_corpus_parses() {
    ...
    let src = include_str!("fixtures/09_refinements/refinements_valid.mvl");
...
fn effect_decl_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/07_effects/effect_decl.mvl");
...
fn subsumption_corpus_checks() {
    ...
    let src = include_str!("fixtures/07_effects/subsumption.mvl");
...
fn user_defined_effects_corpus_checks() {
    ...
    let src = include_str!("fixtures/07_effects/user_defined_effects.mvl");
    let result = check_src(src);
...
fn concurrency_effects_corpus_checks() {
    ...
    // THEN: no effect errors
    let src = include_str!("fixtures/07_effects/concurrency_effects.mvl");
...
fn pure_vs_effectful_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/07_effects/pure_vs_effectful.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn propagation_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/07_effects/propagation.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn totality_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/10_termination/total_vs_partial.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn capabilities_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/12_actors/capabilities.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn session_types_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/12_actors/session_types.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn labels_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/08_ifc/labels.mvl");
    let result = check_src(src);
...
fn label_types_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/08_ifc/label_types.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn lattice_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/08_ifc/lattice.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn propagation_ifc_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/08_ifc/propagation.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
        "propagation corpus should type-check cleanly, got: {:?}",
...
fn declassification_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/08_ifc/declassification.mvl");
    let result = check_src(src);
    let serious_errors: Vec<_> = result
        .errors
        .iter()
...
fn secret_env_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/08_ifc/secret_env.mvl");
    let result = check_src(src);
    let serious_errors: Vec<_> = result
        .errors
        .iter()
...
fn capability_labels_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/08_ifc/capability_labels.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn literals_corpus_with_multiline_raw_strings_checks() {
    let src = include_str!("fixtures/01_syntax/literals.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn implicit_flow_corpus_has_violations() {
    let src = include_str!("fixtures/08_ifc/implicit_flow.mvl");
    let result = check_src(src);
    assert!(
        result
            .errors
...
fn cross_function_implicit_corpus_has_violations() {
    ...
    let src = include_str!("fixtures/08_ifc/cross_function_implicit.mvl");
    let result = check_src(src);
    assert!(
        result
            .errors
...
fn interprocedural_taint_corpus_has_violations() {
    ...
    // THEN: CrossFunctionImplicitFlowViolation is emitted
    let src = include_str!("fixtures/08_ifc/interprocedural_taint.mvl");
    let result = check_src(src);
    assert!(
        result
            .errors
...
fn return_label_inference_corpus_has_no_req11_violations() {
    ...
    let src = include_str!("fixtures/08_ifc/return_label_inference.mvl");
    let result = check_src(src);
...
fn interprocedural_clean_corpus_has_no_req11_violations() {
    ...
    // THEN: no ImplicitFlowViolation or CrossFunctionImplicitFlowViolation
    let src = include_str!("fixtures/08_ifc/interprocedural_clean.mvl");
    let result = check_src(src);
    let violations: Vec<_> = result
        .errors
        .iter()
...
fn call_chain_error_names_callee_and_observable() {
    ...
    let src = include_str!("fixtures/08_ifc/call_chain_error_message.mvl");
    let result = check_src(src);
...
fn refinements_violations_corpus_no_static_violations() {
    ...
    let src = include_str!("fixtures/09_refinements/refinements_violations.mvl");
    let result = check_src(src);
    assert!(
        !result
            .errors
...
fn file_io_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/13_stdlib/file_io.mvl");
    let result = check_src(src);
    let serious: Vec<_> = result
        .errors
        .iter()
...
fn logging_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/13_stdlib/logging.mvl");
    let result = check_src(src);
    let serious: Vec<_> = result
        .errors
        .iter()
...
fn unix_process_lifecycle_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/13_stdlib/unix.mvl");
    let result = check_src(src);
    let serious: Vec<_> = result
        .errors
        .iter()
...
fn unsigned_types_corpus_parses_and_checks() {
    let src = include_str!("fixtures/04_primitives/unsigned_types.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn bit_operators_corpus_parses_and_checks() {
    let src = include_str!("fixtures/04_primitives/bit_operators.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn overflow_checking_corpus_parses_and_checks() {
    let src = include_str!("fixtures/04_primitives/overflow_checking.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn contracts_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/11_contracts/basic_contracts.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn ghost_corpus_parses_and_checks() {
    ...
    // THEN: no type errors
    let src = include_str!("fixtures/11_contracts/ghost_old_contracts.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn loop_verification_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/11_contracts/loop_verification.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
...
fn keywords_corpus_parses_and_checks() {
    ...
    let src = include_str!("fixtures/01_syntax/keywords.mvl");
    let result = check_src(src);
    let serious: Vec<_> = result
        .errors
        .iter()
...
fn generic_instantiation_corpus_parses_and_checks() {
    let src = include_str!("fixtures/02_functions/generic_instantiation.mvl");
    let result = check_src(src);
    assert!(
        result.is_ok(),
