...
- **`vendor/mvl-spec` bumped 0.1.4 → 0.1.7, `vendor/tree-sitter-mvl` bumped to v0.1.7.** Picks up the numeric-literal EBNF completeness fix (mvl-spec#38): `INTEGER`/`FLOAT` productions previously had no representation at all of hex/binary/octal literals (`0x`/`0b`/`0o`) or scientific exponents (`1.5e10`), despite the lexer (`src/mvl/parser/lexer/numbers.rs`) already implementing both. Investigating what depended on that EBNF turned up real, previously-untested parser/lexer gaps across the whole toolchain: this repo had zero test coverage for these numeric forms — added `#[test]`s in `src/mvl/parser/lexer/mod.rs` and a new `tests/corpus/00_smoke/numeric_literals_test.mvl`; `tree-sitter-mvl`'s `integer_literal`/`float_literal` were decimal-only regexes — `0xFF`/`1_000`/`1e10` all misparsed (fixed upstream, mvl-lang/tree-sitter-mvl#7); `mvl-spec`'s Pygments lexer was missing an octal pattern and a dot-less-exponent float pattern, and VS Code's TextMate grammar supported only plain decimal (fixed upstream, mvl-lang/mvl-spec#56). Also fixes a Rust-backend codegen bug found along the way: `push`/`extend`/`append` on a method parameter never marked it `mut` in the generated Rust signature, breaking every builtin body using them (`self_.push(...)` failed to compile with `cannot borrow as mutable`) — this affected the Rust backend for essentially any nontrivial program, not just the numeric-literal work.
