...
- **WASM: `List[List[T]]::sort()` (scalar-`T` inner lists) sorted by pointer, not content** — `bwt.mvl::bwt_encode` sorts `List[List[Byte]]` rotations and depends on genuine lexicographic order to find the correct primary index; pointer order gave a silently wrong (not even nondeterministic, just wrong) result. Added `_mvl_array_sort_nested_bytelist`, reordering the outer array's element pointers in place by comparing each inner array's byte content.
