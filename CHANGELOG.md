...
- **Any `struct` used as a collection element was silently truncated to its first 8 bytes.** Two size functions existed: `alloc_size_for_llvm_ty` computes a real alignment-aware struct size from `struct_fields`, while a static `llvm_type_size` returned a flat 8 for any `%Name` ("conservatively use pointer size"). Every collection call site used the static one, so a `List[Rec]` over a 40-byte `%Rec = type { ptr, i64, double, { i8, ptr } }` was created with `elem_size 8` — field 0 read back correctly, every later field returned garbage. Reproduces with no higher-order function involved, just `mk().get(1).amount`. All five element/value-size call sites now route through the alignment-aware version and the static one is deleted, so the two can no longer disagree.
- **A tail-position `match`/`if` *statement* returned a value it had just freed.** `exclude_returned_value_tir` covers `Var`/`Consume`/`Relabel`/`Construct`, and `emit_block_tir_typed`'s tail-*expression* arm calls it — but a tail `match` lowers to `TirStmt::Match`, which went straight to emission. `match opt { Some(s) => s, None => d }` as a whole function body emitted `phi ptr [ %t2, some ], [ %default, none ]` followed by `_mvl_string_drop(ptr %default)` *before* the `ret`, so on the `None` path the caller's first use of the returned string read freed memory. Excluding every arm is deliberate — which arm runs is a runtime property, and the untaken arms' values are unreachable, so over-excluding costs at worst a leak, never a double free (the same trade `Construct` already makes).
- **`fold`'s empty accumulator literal had no element-size source.** `records.fold([], |acc: List[Rec], r: Rec| ...)` has no element to infer from and, unlike a `let` or a struct field, no declared type at that position either, so its `.ty` stays `List[Unknown]` and it fell back to the 8-byte default — then got `concat`-ed with correctly sized 40-byte arrays built inside the closure, mixing strides in one buffer. The closure's own first parameter carries the exact accumulator type and is now used as the hint.
- `examples/task_pipeline` under `--backend=llvm`: **36/51 → 51/51**, and gains a `test-llvm` target. The wasm side stays at 36/51 for unrelated reasons (its failures span `parser_test.mvl`, and it sizes struct fields correctly) — tracked separately, no `test-wasm` target added.

### Fixed — #2285 (LLVM + WASM)

- **WASM: `assert_eq`/`assert_ne` on `Option[T]`/`Result[T, E]` compared boxed handles by pointer identity.** #2249 gave the `==`/`!=` *operator* content-comparison arms for these types; `emit_assert_eq` never got them and fell through to the generic `i32.eq`. Two structurally equal Options built independently (`row.get(1)` vs a fresh `Some("x")`) are never the same pointer, so every such assertion trapped — and `assert_eq(row.get(1), Some("x"))` is the natural way to assert on any `.get()`/`.first()`/`.last()` result. Reuses the existing `_mvl_option_eq`/`_mvl_result_eq` helpers; no new runtime surface.
- **LLVM: `List[T]::concat` aliased pointer-shaped elements.** `_mvl_list_concat` byte-copies both inputs' element bytes, which for a pointer element hands the result the same heap objects its inputs still own — whichever is dropped first frees them out from under it. Exactly the defect `_mvl_list_slice` had before #2265, with the same fix shape: `_mvl_list_concat_ptr` (generic, taking the matching `_mvl_*_clone` as a callback) and `_mvl_list_concat_enum` (taking the per-type clone trampoline), dispatched on the element `Ty`. Scalar elements keep the cheap byte-copy path. `std/csv.mvl::parse_rows_with` accumulates `rows = rows.concat([row])` on a `List[List[String]]` and hit it directly.
- **LLVM: a name bound out of an enum variant's payload was freed by a consuming callee.** `resolve_owned_call_arg` decides "is this heap-owned?" by looking the binding up in `heap_locals`, but a payload-bound name lives in `locals` only — deliberately not scope-drop-tracked, since the payload buffer stays the owner. The lookup found nothing, so a non-last use was passed *by move* to a consuming parameter that then freed it. `parse_rows_with`'s `is_empty_row(row)` … `rows.push(row)` is exactly this: `is_empty_row` takes `List[String]` consuming and emits `_mvl_string_ptr_array_drop(%row)` before returning, so every parsed row was freed mid-loop and `rows` ended up holding dangling inner pointers — `.len()` still read correctly while `.get(0)` faulted. Now falls back to the argument's own type, mirroring the adjacent `FieldAccess` arm; still gated on non-last-use.
- **LLVM: `assert_eq`/`assert_ne` on `Option[T]`.** Previously a hard codegen error — "unsupported LLVM type `{ i8, ptr }` for compared values" — which, because it aborts emission, failed the *whole file* rather than one assertion. Compares discriminants, then payloads only when both are `Some` (a `None`'s payload pointer is null, so an unconditional load would fault), reusing the same per-type payload strategy as the non-Option arm. `Result[T, E]` stays unsupported by design: its two arms carry different payload types, so one payload comparison cannot cover both.
- `examples/csv_transactions`: **3/5 → 5/5 on both backends**, and gains `test-llvm` + `test-wasm` targets.

### Fixed — #2265 (LLVM)

- **No generic deep-clone existed for struct/enum elements, blocking `List[HuffmanTree]::extend` and friends.** Note the issue's suggested direction — mirror `emit_drop_for_heap_local`'s per-`HeapKind` drop dispatch — does not apply: `heap_kind()` returns `None` for every `Ty::Named`, so struct/enum locals and `Box[T]` are not drop-tracked at all (they leak; there is no drop walker to mirror, and no `_mvl_box_drop` exists). Added instead a lazily generated, memoized `ptr @_mvl_clone_enum_<Name>(i8 disc, ptr payload)` trampoline per payload enum, switching on the discriminant and cloning each variant's fields out of the same flat `[n x i64]` payload-slot buffer the bind/construct paths already walk. Memoized *before* the body is built, so a self-referential field (`Box[HuffmanTree]` inside `HuffmanTree`) recurses through its own symbol instead of looping the generator. Per-field recursion: scalars copy bits; `String`/`List`/`Set`/`Map` take the existing shallow refcount clone; `Box[T]` allocates a fresh box and recurses; a nested payload enum recurses via the trampoline. Wired into `List[T]::extend` (new `_mvl_array_extend_enum` runtime helper taking the trampoline as a callback, mirroring `_mvl_array_drop_option`'s existing callback-pointer pattern), into `slice`/`take`/`skip`, and into `get`/`first`/`last`'s clone-on-extract.
- **`_mvl_list_slice` aliased pointer-shaped elements.** It byte-copies each element — correct for scalars, but for a pointer element the slice and its source both own the same heap object. `String` got its `_str` variant in #2260; nested `List`/`Set`/`Array`/`Map` and payload enums had nothing, so `examples/bzip/huffman.mvl::remove_at_ll`'s `list.slice(..)` on a `List[List[Int]]` double-freed every inner list. Added `_mvl_list_slice_ptr` (generic, clone callback) and `_mvl_list_slice_enum`.
- **`let x: ref T = <heap local>` tracked one allocation twice.** The binding aliases its initializer rather than copying it, but pushed a second `heap_locals` entry without releasing the source's — so the scope-exit sweep dropped it twice, and when the binding was the return value the stale entry freed it before the caller read it (`huffman.mvl::remove_at_ll` returns exactly that shape). Ownership now transfers, mirroring what `Assign` (#2260) and the push/`Some`/`Ok`/`Err`/list-literal/struct-literal sites (#1991/#2264) already do. The `needs_deep_copy` path is exempt — it makes an independent copy, so the source keeps its own drop.
- **A `ref` binding leaked across sibling branches.** `emit_expr_tir`'s `Var` arm consults `ref_locals` before `locals`, and neither map is scoped to the block that introduced its entry, so a `ref` binding left over from an already-finished sibling branch captured every later mention of that name — including in a branch that declared its own plain local. `huffman.mvl::build_tree` declares `codes` as `ref` in one arm and plain in the next, so the else arm's `BuildState { .., codes: codes }` compiled to a load from the *then* arm's alloca, never stored to on that path: uninitialized stack memory reached `_mvl_array_clone`/`_mvl_array_len` as a misaligned dereference while the else arm's own freshly-mapped list was dropped unused. Both `let` arms now clear the other map's entry.
- `examples/bzip` under `--backend=llvm`: **53/71 → 71/71**, exact parity with `--backend=rust`. This also resolves **#2209** (`roundtrip_single_byte`/`roundtrip_all_same_bytes` hanging indefinitely at 100% CPU) — the whole file now runs in ~1s. The example had *no* `test-llvm` target, and `examples/test-all.sh` silently skips any example lacking the target for the backend being run, which is why the breakage went undetected by the example suite on every commit; the target is now added.
- Still out of scope, unchanged: deep clone for *struct*-typed (as opposed to enum-typed) list elements, which errors explicitly rather than miscompiling, and the fact that struct/enum locals and `Box[T]` are not drop-tracked at all.

### Fixed — #2271 (WASM)

- **`List`/`Set[String]::contains` picked the wrong native dispatch.** The arm chose between `_mvl_array_contains_i32`/`_i64` from `is_i32(&elem_ty, ctx)`, with no `String` case at all — and `is_i32` correctly says no for a `String` element (a boxed `*MvlString` handle), so it fell to the i64 arm and was called with an i32 handle: a module-validation type mismatch, not a runtime bug. Added `_mvl_array_contains_str`, which unboxes each stored handle and compares via the existing `_mvl_string_eq`. `List` and `Set` share the arm, so one runtime function fixes both.
- **A string literal used only inside a HOF closure body was never interned.** `collect_literals`' pre-pass walks every fn body to intern literals into the data section, but its `collect_expr` walker never recursed into `TirExprKind::Lambda` — a closure's body is emitted later, as its own top-level `$__lambda_…` fn. A literal referenced *only* inside a closure (e.g. `xs.any(|s: String| s == "zzz")`) was therefore interned by nobody: its use site emitted `;; missing literal`, a stack-underflow module `wasm-tools parse` accepts but `validate` rejects.
- **`List[T]::min()`/`::max()` were silently wrong for every element type.** WASM had no native min/max dispatch, so every call monomorphized `std/lists.mvl`'s documented fallback body (`self.first()`/`self.last()`, not a true min/max — see that file's own doc comment). The issue reported this for `String` only; the same fallback applies to every element type, so `List[Int]::min()` was equally wrong. Added `_mvl_list_min_index_i64`/`_max_index_i64` and `_mvl_list_min_index_str`/`_max_index_str`, mirroring the LLVM backend's existing helpers.

### Fixed — #2272 (checker, resolver)

- **`assurance`/`prove`/`harden` still used the whole-directory prelude.** #2204 closed this for `check`/`test`, but the identical `all_user_progs.split_at(idx)` construction was copy-pasted independently into three more CLI commands, each of which also called `loader::load_sibling_modules_transitive` in its single-file branch only — the same asymmetry `check.rs` had. All four now route through one shared per-file prelude helper rather than a fourth copy.
- **Parameter and return type names were never validated.** A completely fictional type in parameter position compiled clean, with no such type anywhere in the project — the most common annotation position, and a genuine Req 1 hole (constructor position *was* checked, which is what made it asymmetric). Also closes duplicate #2279.
- **Brace-group `use` mis-resolved a deeper qualified module path.** `use <a>.<b>.{X, Y}` failed when segment `<a>` independently named a real module file, while the single-item `use <a>.<b>::X;` form resolved the identical path — the resolver read `solver.layer2.{Bound}` as "item `layer2` of module `solver`". Only the ≥2-segment-path-with-a-real-prefix combination was affected.

### Fixed — test fixtures

- Repaired type-checker fixtures that the parameter/return-type validation above legitimately started rejecting: `SINK_PRELUDE` declared `write_file`/`append` taking a `Path` without declaring `Path`; the dead-letter corpus test loaded five stdlib modules but not `std/io.mvl`, which `log.mvl`'s `use std.io.{Fd}` needs; and two corpus fixtures used illustrative placeholder type names (`Channel`, `Payload`, `Token`, …) that were never declared. Also regenerated `tests/checker_parity/baseline.tsv`, where `list_get_field_receiver.mvl`'s error count moved 1 → 3 (same requirement, still `fail`) — that harness's prelude does not do sibling-module resolution, so the file's `use` of a sibling type was already a documented failure case and the new validation simply reports the resulting undefined types too.
...
- **`Map[K, V]::get()` double-freed a pointer-shaped `V`.** The clone-on-extract fix from #2047 only covered `V == String`; `Map[Int, List[Int]]::get()` (as produced by `.group_by(...)`) handed back the same `List[Int]` pointer the map's own slot still owned, so extracting it into a local and later dropping both the local and the map double-freed it — same bug class as #2203/#2252/#2264. `List::get`'s equivalent arm already used a generalized `elem_clone_sym()` helper (String/List/Array/Set/Map); `Map::get` now reuses it instead of a String-only check.

### Changed (breaking) — #2204

- **`mvl check <dir>` and `mvl test <dir>` now require an explicit `use` for every cross-file reference, matching `mvl build` and single-file mode.** Directory mode silently exempted any sibling file that declared a `type` or `extern` block: its `pub` items became visible to every other file in the directory with no `use` statement anywhere. `.openspec/specs/005-modules/spec.md` Req 2/3 grant no such exception — visibility (`pub`) and reachability (`use`) are two separate, both-mandatory steps, as in Rust and Python (Go is the outlier, and MVL's design table explicitly chose against it). The exemption came from two places: `check.rs`'s directory-mode "whole-program checking" handed every file all other files as its prelude, and `test.rs`'s sibling gate was `has_extern_or_type_decls(&parsed) || imported_by_test_files.contains(...)` — the `||` bypassing, for type/extern-declaring files, the import check that already correctly gated plain-function siblings. Both now build each file's prelude from its own `use`-based transitive closure via the existing `loader::load_sibling_modules_transitive` (anchored on the CLI's base directory, so a nested file's bare reference to a top-level module still resolves). **Migration:** add the missing `use` statements — the compiler now names each one. `compiler/` needed ~30 files' worth (`compiler/solver/` and 22 of 32 top-level files had zero `use` statements and compiled only because of this bug).
- **New methods-only prelude channel reconciles this with Go-model method dispatch (#1706).** Extension methods on a shared receiver type can be split across sibling files that call each other in a cycle (`EmitCtx::emit_expr` in `emit_exprs.mvl` ↔ `EmitCtx::emit_body` in `emit_stmts.mvl`), and spec 005-modules bans circular `use` imports — so that cycle cannot be expressed as an import graph at all. `mvl build` never had to confront this (it reports diagnostics only for its single entry file, so sibling-dispatch errors were swallowed); `mvl check <dir>` must. `check_with_two_preludes_and_methods_mode` takes a third prelude from which `collect_methods_only` registers *only* `Decl::Fn` declarations carrying a receiver type — a Go-model sibling's types, free functions, consts and externs stay invisible without an explicit `use`, so the hole this issue closed stays closed. The channel covers **builtin** receivers (`String::`, `List::`, `Map::`, …) as well as user-defined ones — builtins never appear in the checker's type table, so the receiver-visibility test shares `BUILTIN_RECEIVER_TYPES` with `register_fn` rather than carrying its own copy. Seven unit tests plus `tests/check_sibling_visibility.rs` pin both halves of the contract, including the mutually-recursive sibling case the channel exists for. This supersedes the "per MVL's same-directory implicit visibility" characterization in the #2207/#2200 note under 1.7.4 below: that behaviour was this bug, not a language rule.
- **Method dispatch is now invocation-mode independent.** `mvl check <file>` previously saw no sibling extension methods at all, so a file could pass `check <dir>` and fail `check <file>` (or vice versa) purely on how it was invoked — the same class of mode-dependence this issue set out to remove. Single-file mode now receives the same methods-only prelude as directory mode. Cross-file *types* and *free functions* remain gated on an explicit `use` in both modes.
- **Also fixed, uncovered by the above:** files carrying inline `test fn`s are bundled into the test crate by their own pass, independently of `*_test.mvl` discovery, but only `fn main` entry points had their imports folded into `imported_by_test_files` — so an inline-test file's `use` was ignored and its imported sibling never loaded (visible as a `mvl check`-passes / Rust-backend-fails split on `tests/corpus/05_collections/list_get_field_receiver.mvl`). The removed `has_extern_or_type_decls` hatch had been masking it.

### Fixed — #2264 (LLVM)

- **LLVM: `List[List[Byte]]`/`List[List[Bool]]::sort()` sorted by inner-array pointer identity, not content.** Same bug class as WASM's `_mvl_array_sort_nested_bytelist` (#2267): `_mvl_list_sort` compares elements as raw i64 bit patterns, which for a nested-list element is the inner array's heap address, so `examples/bzip/bwt.mvl::bwt_encode`'s rotation sort — which needs genuine lexicographic order to find the correct primary index — got a silently wrong result rather than a crash. Added `_mvl_list_sort_nested_bytelist`, scoped to `Byte`/`Bool` inner elements only: their LLVM element type is `i8` (`elem_size == 1`), so raw byte-lexicographic comparison is exactly element-wise order; `Int`/`Float` inner lists are multi-byte lanes where that equivalence breaks down, and stay on the pre-existing pointer-order fallback. Extended `list_sort_test.mvl`'s shared (LLVM + WASM) coverage with a lexicographic-prefix case — a comparator that only looked at element 0, or compared lengths first, would pass the existing test but still give bzip the wrong primary index.
...
- **LLVM: `List[T]::extend` silently vanished** — same "no dedicated dispatch arm beyond the receiver's own `T`" bug class as #2256. Fixed for scalar/String/`List[U]`-of-scalar element types; the `List[HuffmanTree]` (recursive-enum element) case needs the generic deep-clone support tracked separately in #2265.
- **LLVM: `==`/`assert_eq` on `List`/`Set`/Array-shaped values compared by raw pointer, not content** — both `emit_binary_tir` and `emit_assert_eq_builtin_tir` matched on the LLVM-lowered type string `"ptr"` alone (true for `String`, `List`, `Set`, and `Map` alike) and unconditionally called `_mvl_string_eq`, reading an `MvlArray*`'s bytes through `MvlString`'s field layout (no `elem_size` field) — silent memory corruption for any non-String pointer-shaped comparison. Now gated on the actual checker `Ty`, dispatching to `_mvl_array_eq` for scalar-element `List`/`Array`/`Set`.
- **LLVM: `.map()`'s output array was sized from the *input* list's element type, not the closure's return type** — `List[Int]::map(|v: Int| -> Byte {...})` (bzip's universal pattern for building `List[Byte]`) produced an 8-byte-strided array instead of 1-byte, corrupting every read past the first element.
- **LLVM: empty `List`/`Set` literals (`[]`) defaulted their element size to 8 bytes ("ptr") unconditionally** — an empty literal's own checker-resolved type is `List[Unknown]`, so the emitter couldn't infer the real element size from it. Fixed by reading the declaring `let`'s or struct field's own declared type instead, at both the `let x: T = []` and `Struct { field: [], .. }` call sites.
- **LLVM: four distinct use-after-free/double-free gaps in the ownership/drop-tracking machinery**, found via a minimal repro isolated from `examples/bzip`'s `bwt.mvl`/`bitstream.mvl`: (1) `resolve_owned_call_arg` only recognized a bare `Var` call argument as needing a non-last-use clone, never a `FieldAccess` (`owned.data` read multiple times); (2) `exclude_returned_value_tir` never recursed into a `Construct`/`Spawn` field value, so a struct literal returned as a function's tail expression could have one of its own fields double-freed; (3) `emit_construct_tir` never called `exclude_returned_value_tir` on any of its field values at all (unlike the list-literal emitter, which always has) — a local moved into a struct field and then returned through a recursive rebuild got dropped out from under the in-flight return value; (4) `emit_block_tir_typed`'s tail-expression handling excluded a `FieldAccess` tail from its own block's scope-exit drop sweep but never a bare `Var` tail — a `ref`-qualified local returned as an if/else branch's tail value (`out.push(b); out`) got dropped by that branch's own sweep right before its value fed the merge `phi`.
- Added C-ABI runtime helpers in `runtime/llvm/src/memory_ops.rs`: `_mvl_array_eq`, `_mvl_array_extend`/`_str`/`_nested`, `_mvl_list_sort_bytelist`, `_mvl_list_join_str`, `_mvl_array_contains_str`, `_mvl_list_{min,max}_index_{i64,str}`, `_mvl_array_reverse`, `_mvl_list_reverse_str`.
- `examples/bzip` now passes 53/71 tests under `--backend=llvm` (up from a crash/hang on essentially every test before this fix). The remaining 18 failures (`huffman_test.mvl`, and every `roundtrip_test.mvl` case that depends on Huffman tree construction) all trace to #2265 (`List[HuffmanTree]::extend` needs generic recursive-enum deep-clone support), not this issue — a `test-llvm` Makefile target for `examples/bzip` is deferred until that lands and the suite is fully green. The WASM side of the original combined issue (`ll_set`/`remove_at`/`remove_at_ll` trapping on compound element types) is split out to #2267.

### Fixed — #2267 (WASM)

- **WASM: `List[T]::extend`/`.slice()` traps on compound (nested-collection or struct/payload-enum) element types** — `huffman.mvl`'s `ll_set`/`remove_at`/`remove_at_ll` (Huffman priority-queue merge loop, hit on any multi-symbol input) compiled to `unreachable` stubs. `slice_is_supported`'s gate excluded any pointer-shaped element as unsafe-to-alias, following `List[String]`'s reasoning — but WASM's `local_drop_fn` only deep-drops `String` elements; nested-collection/struct/payload-enum elements fall through to a shallow `_mvl_array_drop` that never touches them, so aliasing across a `.slice()` is exactly as safe (and exactly as leaky, a separate pre-existing gap) as leaving them untouched. Relaxed the gate accordingly.
- **WASM: `==`/`!=`/`assert_eq`/`assert_ne` on scalar-element `List`/`Array`/`Set` compared by pointer, not content** — every collection type answers `true` from `is_i32` (a pointer, i32 on wasm32), so the generic-op fallback picked plain `i32.eq` on the two array handles themselves. Same bug class the LLVM backend had for the same operators (#2264). Added `_mvl_array_eq` (content comparison at `elem_size` granularity) and a dedicated dispatch arm; String/nested-collection/struct/payload-enum elements are left on the pre-existing (still wrong) fallback, not yet covered.
- **WASM: `List[List[Byte]]`/`List[List[Bool]]::sort()` sorted by pointer, not content** — `bwt.mvl::bwt_encode` sorts `List[List[Byte]]` rotations and depends on genuine lexicographic order to find the correct primary index; pointer order gave a silently wrong (not even nondeterministic, just wrong) result. Added `_mvl_array_sort_nested_bytelist`, reordering the outer array's element pointers in place by comparing each inner array's byte content. Scoped to `Byte`/`Bool` inner elements only (single-significant-byte, non-negative) — raw byte comparison does not match numeric order for `Int`/`Float` inner lists (multi-byte magnitudes, two's-complement/IEEE-754 sign bits), which stay on the pre-existing pointer-order fallback, still wrong but no worse than before (caught in review before merge — a first version of this dispatch mis-routed any scalar inner type here).
- **WASM: empty `List`/`Set` literals (`[]`) defaulted their element size to 8 bytes regardless of context** — an empty literal's own checker-resolved type is `List[Unknown]` (no elements to infer from), so `elem_size_bytes` picked the wrong stride, corrupting every later `.slice()`/`.concat()`/etc. that trusts the array's own stored `elem_size`. Fixed at three call-site shapes that each need to supply the *real* element type from context instead: a struct-construct field's declared type (`RleEncState { out: [], .. }`), a `let`'s own declared type (`List[T]::map[U]`'s `let result: ref List[U] = [];`), and — new registry (`Ctx::fn_param_tys`, every top-level fn's declared param types) — a bare call argument with no enclosing `let`/struct-field context at all (`huf_dec_loop(tree, reader, [], 0, n)`'s `[]`).
- **WASM: a struct-construct field value that moves a named local's value in (`RleEncState { out: new_out, .. }`) never excluded that local from the function's own exit-time heap-drop sweep** — same "moved into a container but still independently drop-tracked" shape the list-literal/`.push()` emitters already handled, just missing for `emit_struct_construct`'s own fields. Without it, `rle.mvl::rle_emit_byte`/`rle_emit_run`'s returned struct held a field pointing at an array this same function had just freed.
- `examples/bzip` goes from `ll_set`/`remove_at`/`remove_at_ll` trapping on any real input to `bitstream_test.mvl`, `bwt_test.mvl`, `huffman_test.mvl`, `imports_test.mvl`, `mtf_test.mvl`, and `rle_test.mvl` all passing in full under `--backend=wasm`. `roundtrip_test.mvl`'s 8 full compress→decompress pipeline tests still fail on a distinct, not-yet-root-caused bug (deterministic wrong output through a `partial fn` wrapper that disappears when print statements are added nearby) — split to #2274. Closes the WASM half of #2264 (split from #2267) short of that remainder.

### Fixed — #2256

- **LLVM: `List[String]` operations broken across the board** — `join`, `contains`, `last`, `min`, `max`, `is_empty`, `reverse`, and `rev` all silently emitted nothing (or, for `contains`, silently gave the wrong answer) on `--backend=llvm`. Root cause for six of the seven: each is a non-generic extension method (no bracket of its own beyond the receiver's `T`), so it fell outside `emit_method_call_tir`'s generic-fallback dispatch and had no dedicated arm — the call just vanished, with no crash and no diagnostic. `is_empty` turned out broken for every receiver type, not just `List[String]`. `contains` compared string pointer identity instead of content. Direct IR inspection showed most of the issue's originally-reported "14-op String-drop crash class" wasn't a refcount bug at all — `concat`/`filter`/`skip`/`take`/`take_while`/`skip_while`/`Bool::to_string()` already computed correct results once `join` was fixed; the survey had likely rendered `List[String]` results via `.join(",")`, so the join bug was masquerading as those crashes. Added dedicated C-ABI symbols (`_mvl_array_contains_str`, `_mvl_list_join_str`, `_mvl_list_{min,max}_index_{i64,str}`, `_mvl_array_reverse`, `_mvl_list_reverse_str`) plus a 27-test corpus regression file. `examples/mastermind` (the original report site) now passes 18/18 under `--backend=llvm`. `List[T]::sort_by` (zero LLVM dispatch for any element type) and `List[String]::skip`/`::take`/`::slice` on WASM (module validation failure) were found during this work but are out of scope — split to #2260 and #2262.

### Changed — #2254

- **`vendor/mvl-spec` bumped 0.1.4 → 0.1.7; `vendor/tree-sitter-mvl` bumped to v0.1.7.** Picks up the numeric-literal EBNF completeness fix (mvl-spec#38): `INTEGER`/`FLOAT` productions previously had no representation at all of hex/binary/octal literals (`0x`/`0b`/`0o`) or scientific exponents (`1.5e10`), despite the lexer (`src/mvl/parser/lexer/numbers.rs`) already implementing both — a documentation-only gap in the spec, not a compiler behavior change. `make test-grammar-coverage` failed after the bump: the new `HEX_DIGIT`/`BIN_DIGIT`/`OCT_DIGIT`/`EXPONENT` EBNF terminals had no tree-sitter counterpart in the cross-check's known-absences list. Documented in `tools/check_grammar_coverage.py`'s `EBNF_KNOWN_ABSENT` — but this surfaced a second, real bug along the way: `tree-sitter-mvl`'s `integer_literal`/`float_literal` regexes were decimal-only, not actually covering hex/binary/octal/exponent at all despite the doc-fix's assumption otherwise (mvl-lang/tree-sitter-mvl#7, released as v0.1.7). Also caught downstream: Pygments was missing an octal pattern and a dot-less-exponent float pattern, and VS Code's TextMate grammar supported only plain decimal — both fixed in mvl-spec 0.1.7, which also re-pins `editors/zed/extension.toml`'s tree-sitter-mvl rev to the fixed grammar.
- **Added regression coverage for the numeric-literal forms above**, previously untested anywhere in this repo: `#[test]`s in `src/mvl/parser/lexer/mod.rs` for `_` separators (decimal/hex/binary/octal/float/exponent), the documented trailing-`_`-compiles gap, and the `0x_`-errors case; a new `tests/corpus/00_smoke/numeric_literals_test.mvl` exercising the same forms end-to-end through the real compiler.
- **Also fixes a Rust-backend codegen bug found along the way**: `push`/`extend`/`append` on a method parameter never marked it `mut` in the generated Rust signature, breaking every builtin body using them (`self_.push(...)` failed to compile with `cannot borrow as mutable`) — affected the Rust backend for essentially any nontrivial program, not just the numeric-literal work.
...
- **`--backend=wasm --target=wasm-browser` is now implemented**, not just accepted. `runtime/wasm/` cfg-gates its ~3 real OS touchpoints (`SystemTime::now()`, `thread::sleep()`) so the crate also compiles to `wasm32-unknown-unknown`; `std.env`/`std.io` get browser-side stub bodies (`Err`/`None`/empty) since the emitter's runtime-import table is fixed regardless of usage. Reuses the existing host (JS shim + WASI polyfill in `runtime/wasm-browser/runtime.mjs`) instead of `wasm-bindgen` — see ADR-0064 for the rejection rationale. `.cargo/config.toml` gained `[target.wasm32-unknown-unknown] rustflags = ["-C", "link-args=--import-undefined"]` to satisfy the JS-provided imports at link time.
- **`make test-wasm-browser`**: builds and runs mvl-lang/mvl-playground's 6 curated example apps end-to-end through the real `wasm-browser` runtime under Node — the first guarantee that the target actually works against real programs, not just the hand-written smoke fixture (`make test-runtime-wasm-browser`).

### Fixed — #2203

- **WASM/LLVM: `List[T]::get()`/`Set[T]::get()` never refcount-cloned a `String` or nested-collection (`List[U]`/`Set[U]`) element before wrapping it in `Some`.** Safe for scalars/structs/payload-enums (nothing ever deep-drops those), but a genuine use-after-free otherwise: binding the `Some` payload to a plain-named local or match-arm binding gets a value that *does* get deep-dropped at scope exit, double-freeing against whatever else still references that element. Confirmed cross-backend (identical crash on LLVM, which had a String-only partial fix that missed nested collections too); Rust backend is unaffected by construction. Fixed on WASM by cloning the extracted payload (`_mvl_string_clone`/`_mvl_array_clone`) before rewrapping in a fresh `Option`. Not yet fixed: LLVM's identical nested-collection gap, and `Map[String, V].get()`'s identical gap for nested-collection-typed `V` — tracked separately.
...
- **WASM: `std.random.choice()` emitted invalid WASM.** `choice(list)` called `_mvl_random_choice_index` and returned its bare `i64` index directly as if it were the `*MvlOption` pointer `Option[T]` promises — the module assembled but failed `wasm-tools validate` (i32 expected, i64 found) wherever the result was matched. Fixed by stashing the array pointer and picked index in temp locals and feeding both into the existing `_mvl_array_get_option_i32/i64` runtime helper (same OOB-check + Some/None wrap `List::get` already uses) — a negative/empty index falls out to `None` for free. `std.env`/`std.io` were already wired to the WASM emitter in an earlier commit; this closes out the remaining `std.random` gap in #2093's Phase 1 checklist.
...
- **`mvl build --backend=wasm` crashed with a stack overflow on `examples/data_integrity/verify.mvl`.** Any `requires`/`ensures` predicate using a bitwise op (`bit_and`/`bit_or`/`bit_xor`/`bit_not`, #1928) hit an infinite mutual recursion: `emit_ref_val_wasm` and `emit_ref_expr_wasm` each fell through to a `_` fallback arm that calls the *other* function on the exact same unhandled `BitwiseOp`/`BitwiseNot` node — neither ever terminates. `is_runtime_checkable` wrongly reported these (and `FieldAccess`/`Len`/`StringOp`/`RegexMatch`/`Min`/`Max`, none of which the emitter implements either) as runtime-checkable, so `emit_contract_check` walked straight into the loop instead of skipping them the way quantifiers and `ArrayGet` already are. `BitwiseOp`/`BitwiseNot` now lower to real `i64.and`/`i64.or`/`i64.xor`/`i64.shl`/`i64.shr_s` instructions (mirroring the Rust backend's `&`/`|`/`^`/`<<`/`>>`); `ref_expr_wasm_ty` also picked `i32` for these by default, so a `Compare` over a bitwise operand emitted `i32.eq` on two i64 values — a second, independent validation bug once the recursion was fixed. The other six variants are excluded from `is_runtime_checkable` for now (same treatment as quantifiers/`ArrayGet`): the static proof still applies, only the runtime assertion is skipped, since none is exercised by the current corpus. `examples/data_integrity` now builds, validates, and passes all 13 tests via `--backend=wasm`, and gets a `test-wasm` Makefile target wiring it into `make test-examples-wasm`.

### Fixed — #2084

- **`examples/bzip` could not be compiled to WASM at all.** Four causes, each only visible once the one before it was fixed. Two in the example: `fn b` (a test helper) was copy-pasted into `bwt.mvl`/`rle.mvl`/`mtf.mvl`, and same-directory modules share one flat WASM symbol space, so #2036's duplicate detector rejected the build outright. Those in-module test sections were migration leftovers — the `*_test.mvl` siblings already cover strictly more (bwt 3→6, rle 3→8, mtf 3→7), so they were deleted with no coverage lost, and the same helper colliding across the test files was namespaced the way `bwt_test.mvl` already did it. `zero_int`/`empty_code`/`int_list_set`, duplicated verbatim in three files, moved to a shared `intlist.mvl`. Two in the backend, both the #2076 shape (a `builtin fn` every other backend implements, for which WASM silently emitted a bare `call $name`): `from_int`/`wrapping_from_int` now lower inline as `i32.wrap_i64`, mirroring LLVM's `trunc i64 … to i8`; and `Box::new` gained a `_mvl_box_new` runtime shim plus its allocate-tee-store sequence. `Box[T]` also had to be taught to `wasm_ty`/`is_i32` — it fell through the `_ => "i64"` default, so `emit_payload_store` skipped the `i64.extend_i32_u` widen an 8-byte payload slot needs and the module failed to validate even once the symbol existed. bzip now passes `wasm-tools validate`; it does not yet run (23 functions still stub on `List::extend` and capturing closures).
- **`std.env`'s `args()` and `get()` had no WASM runtime shim** — same gap #2076 closed for `read_file` and #2055 tracks for `exit`. `args` alone blocked three examples (csv_transactions, log_analyzer, task_pipeline); `get` blocked log_to_file. `runtime/wasm` targets `wasm32-wasip1` with `std` and already used `std::env::var`, so both shims make the same calls the Rust backend does. The `get` arm matches std.env's *shape* — one String argument and an `Option` result — rather than the bare name, so a user-defined `fn get(s: String) -> Int` is not hijacked; there is a regression test for that. These unblock one layer each rather than whole examples: every one advances to its next distinct gap, surveyed in #2084.

### Added

- `examples/**/*.wat` is git-ignored. `mvl build --backend=wasm` writes `<stem>.wat` into the working directory, so building an example in place left the artifact behind. Scoped to `examples/` rather than a blanket `*.wat`, because the reference modules in `tests/spikes/006-wasm-backend/` are checked in on purpose.
...
- **`examples/programs/auth_handler.mvl`'s `// Expected stdout:` annotation had an explanatory parenthetical baked into the expected value itself** (`... (now_plus() is a stub returning 0)`), which no backend's actual stdout ever produced. Moved it to a plain comment above the block.

### Fixed — #2075

- **Match dispatch on an enum with a struct-shaped payload variant crashed both the LLVM and WASM backends.** Root cause was three-layered, one per compiler phase. (1) The checker's `bind_match_pattern` bound every `Pattern::Struct` field to `Ty::Unknown` unconditionally — unlike the `TupleStruct` arm right above it, it never looked up the variant's declared field types, silently breaking any downstream use of the binding. (2) LLVM's bare-enum match never recognized `Pattern::Struct` as a discriminant-bearing pattern (silently fell through to wildcard treatment, returning the wrong value), and its `Err(Enum::Variant)` two-level dispatch typed its inner switch condition as the wrapped enum's own tagged-union type (`{i8,ptr}`) instead of extracting its discriminant byte first — LLVM rejects a non-integer switch condition outright (`switch condition must have integer type`), `examples/programs/auth_handler.mvl`'s exact crash. (3) WASM's `Err(inner)` arm checked only the outer `Result` tag, never the wrapped enum's own discriminant — multiple qualified `Err(Enum::Variant)` arms in one match all lowered to the same `tag == 1` condition, so only the first one listed was ever reachable; neither `emit_match_impl` nor the match-arm local-collection pass had any `Pattern::Struct` handling at all, in either the bare-enum-match or `Err`-wrapped path. Fixed all three layers; also fixes the sibling #2073 bug (payload-enum `TupleStruct` field locals hardcoded to `Ty::Int` in WASM's first, ctx-unaware local-collection pass) via a new ctx-aware correction pass that now wins a "keep last" local dedup (previously "keep first"). `examples/programs/auth_handler.mvl` now passes under `--backend=llvm`; `examples/cbtc_train_presence` now passes under `--backend=wasm` (18/18).

### Fixed

- **Rust backend: an actor's generated state struct, spawn function, and mailbox enum were private-by-default**, breaking any cross-module `actor Name { ... }` construction — every existing example had spawned its actors in the same file that declared them, so this never surfaced before. Also, `fn main()`'s `mvl_join_actors()` injection (#1048) only ever checked whether the *current* file declared actors, never whether it merely spawned one declared in a sibling module — with actor_pingpong's actors moved to a sibling module for testability, the join call stopped being injected, and the process could exit before the spawned actors' background threads finished, producing flaky, truncated, or entirely missing output depending on OS thread scheduling. Fixed both; `examples/actor_pingpong` rewritten with a proper `pingpong.mvl`/`main.mvl`/`pingpong_test.mvl` split and real `test`/`test-llvm`/`test-wasm` Makefile targets (previously a stub echoing a long-resolved "#580 not implemented" message).

### Changed

- **`examples/programs/Makefile`: added `test-rust`/`test-llvm`/`test-wasm` targets**, each running `mvl test <file>` per file (via `--expect` for Rust, `--backend=llvm`/`--backend=wasm` for the others) against a per-backend exclude list (`RUST_EXCLUDE`/`LLVM_EXCLUDE`/`WASM_EXCLUDE`) — mirrors `WASM_CORPUS_EXCLUDE` in the root Makefile. `RUST_EXCLUDE` is empty (all 15 programs pass); `LLVM_EXCLUDE`/`WASM_EXCLUDE` document the known-failing files and why. Picked up automatically by `test-examples-llvm`/`test-examples-wasm` via `test-all.sh`'s existing target auto-discovery.
...
- **`compiler/` (self-hosted MVL compiler): 14 `unused-function` and 3 `zero-arg-literal-fn-as-const` lint warnings cleared.** 11 unused functions across `const_eval.mvl`, `decls.mvl`, `main.mvl`, `mono.mvl`, `parser.mvl`, `verify_session.mvl` promoted to `pub` (substantial ported self-hosting pieces, not called yet); 3 genuinely orphaned test-fixture builders deleted (`mk_arm`/`mk_guarded` in `verify_patterns.mvl`, `mk_partial_fn` in `verify_termination.mvl`). `eval_budget`, `max_paths`, `max_depth` converted from zero-arg `Int`-returning functions to `const`s. Left `lbrace`/`rbrace`/`stripped_sentinel` (all `String`) as functions — converting them to `const` makes the Rust backend emit `"...".to_string()` in a const initializer, which rustc rejects (E0015); the lint's own suggested fix doesn't compile for non-`Copy` return types. Also left the 17 `suggest-if-let` warnings untouched: they're 100% false positives — `if let` desugars to the identical `Stmt::Match` AST shape as an explicit match at parse time, so the lint rule can't tell an already-idiomatic `if let` apart from a match that should become one.

### Fixed — #2076

- **WASM backend: `std.io::read_file`/`_read_file` had no import or runtime shim** — the checker saw them as undefined functions under the lightweight "Transpile"-mode prelude pipeline `llvm_text.rs`/`wasm_text.rs` share (their `pub builtin fn` declarations get stripped from that prelude, with nothing supplying a fallback signature). Registered both in the checker's hardcoded builtin table, mirroring `write`'s existing entry — this also fixes the identical gap on `--backend=llvm`. Added `_mvl_io_read_file` to `runtime/wasm/`, backed directly by `wasm32-wasip1`'s `std::fs::read_to_string`; errors map to the same `NotFound`/`PermissionDenied`/`AlreadyExists`/`Other(String)` `IoError` shape the Rust/LLVM backends already use.
- **WASM backend: `Pattern::Ok`'s match-arm codegen never special-cased a `String` Ok-payload** the way `Pattern::Some` already does — `Result[String, E]` Ok bindings had no prior test coverage (every existing Result-returning builtin was `Ok=Int` or `Err=String`, never `Ok=String`), surfaced by wiring `read_file` through. Fixed by mirroring `Pattern::Some`'s `(ptr, len)` local-splitting for `Pattern::Ok`'s binding.
- **`make check-compiler` (`mvl check compiler/` + `mvl lint compiler/`) had five pre-existing failures on `main`, independent of any in-flight feature work.** Three "use of moved value" errors (`effects.mvl`, `resolver.mvl`, `solver/layer3.mvl` ×2) from a value consumed by `List::push` then reused afterward — fixed with `.concat("")` (String has no `.clone()`) where a genuine independent copy was needed, and reordering where the fix was just moving a read before the consuming push. One dangling-reference error (`infer.mvl`): `let elem_ty: ref Ty = Ty::Unknown;` bound a `ref` directly to a bare enum-literal temporary with no other owner — fixed with an intermediate owned `let`. One totality error (`verify_passes.mvl`): `count_memory_safety_sites` was declared `total fn` while calling `walk_block_vp`, a genuinely `partial` mutually-recursive AST walker (same as `walk.mvl`'s `walk_block`/`walk_expr`, never proven total) — declared `partial fn` to match.

### Added — #2014

- **WASM backend: `List[T]` higher-order methods, `sort_by`/`min_by`/`max_by`, and `String::split` now work — all of `tests/corpus/13_stdlib/` runs under `--backend=wasm`** (11 → 43 passing test fns), and the four remaining `13_stdlib` entries are removed from `WASM_CORPUS_EXCLUDE`. Previously `mvl build --backend=wasm` *succeeded* on these files while stubbing most function bodies to `unreachable`: `list_hof_test.mvl` 9/9 stubbed, `list_sort_by_test.mvl` 9/9, `list_method_fallback_test.mvl` 5/6, `parse_test.mvl` 4/8. The issue attributed this to a missing `call_indirect`; that was one of six distinct gaps.
  - **A whole emission bucket was missing.** `emit_program` partitioned `tir.fns` three ways — no-receiver+non-generic, no-receiver, and non-generic+receiver (added by #2054) — and `fn List[T]::flatten` has *both* a receiver and type params, so every filter rejected it. Every pure-MVL `List[T]` method in `std/lists.mvl` was silently never emitted. Added a fourth bucket, monomorphized per call site like generic plain fns (mirroring LLVM's `mono.tir_generic_fns`) since a generic body has no single WASM signature.
  - **Two receiver-name maps answered `None` for built-in constructors.** The parser records `fn List[T]::first` under `"List"`, but a `List[Int]` receiver has type `Ty::List`, never `Ty::Named("List", _)`. In `cli::wasm_text` this meant `xs.flatten()` produced no entry in `RefCollector::method_refs` at all, so the prelude pull-in loop never even considered lowering the body — and its `type_params.is_empty()` gate then dropped generic methods regardless.
  - **`resolve_ty_param` substituted only at the top level**, so `List[T]` with `T → Int` stayed a bare `Ty::Named("T")` and fell through `wasm_ty`'s `_ => "i64"` default — an array pointer typed as i64. Made recursive; added `unify_ty_params`, which binds a type param from *inside* a receiver (`List[T]` vs `List[Int]`) as `infer_type_subst` cannot.
  - **Function values via an intra-module funcref table.** `(table funcref)` + `(elem)` + one `(type)` per distinct signature; a `Ty::Fn` value is an i32 table index, and `f(x)` inside a monomorphized body compiles to `call_indirect`. Each non-capturing lambda becomes a top-level function. A lambda literal reaches TIR with its return type `Unknown`, so the result type param of `map[U]` is recovered from the lambda's *body* type — without that, instances mangled to `List_map__Int__Unknown`. Chained calls (`xs.filter(..).map(..).fold(..)`) additionally needed the receiver's *resolved* type, since a `MethodCall`'s own `ty` is its callee's declared return type.
  - **Four methods had no dispatch arm at all**: `List[T]::push` (`_mvl_array_push_*` was reachable only from list *literals*, making this a prerequisite for every `std/lists.mvl` body), `.clone()` (used by `filter`/`take_while`/`skip_while` and all three `*_by` methods), `.slice()` (new `_mvl_array_slice` in `runtime/wasm/`, backing `take`/`skip`), and `String::split` (new `_mvl_string_split`). `List[T]::contains` had an arm, but its guard admitted only `Ty::Set`/`Ty::Ref`, so the identical call on a plain `List` stubbed.
  - `ADR-0059` §2's "No `(table …)`, no `(elem …)`, no `call_indirect`" is amended in place with a scope note. Its binding constraint — the preloaded `runtime/wasm` module cannot call back into the emitted module — governs *runtime-driven* dispatch; here the table, its `elem`, the lambda bodies, and every `call_indirect` are all inside the one emitted module. Actor dispatch stays static, and no table crosses the `--preload` boundary.
  - Still excluded, and unchanged: `03_functions/higher_order_test.mvl` and `07_ownership/lambda_capture_test.mvl`, which need *capturing* closures — an environment representation, not just a function pointer.

- **WASM backend: stubbed functions are now reported instead of silent, and `make wasm-stub-report` pins the corpus at zero stubs.** When the emitter meets something it can't compile it discards the whole function body and emits `unreachable` — the module still assembles, `wasm-tools parse` is happy, and `mvl build --backend=wasm` exits 0. A half-implemented method was indistinguishable from a working one at the CLI, so gaps accumulated unnoticed until someone happened to run the file: `List[T]::push` had *no dispatch arm at all* while every program using it still "compiled", and #2014 was filed as "currently partially stubbed" precisely because the build passing is what hid it. `WasmTextCompiler::stubbed_fns()` now records every stubbed function and the CLI prints a warning naming them; the new make target fails if any file in `WASM_CORPUS` stubs, so a gap surfaces in the commit that introduces it rather than being discovered in a batch later. Still a warning and not a hard error at the CLI, because shipping partial WASM support by stubbing siblings is a deliberate, still-useful workflow.
- **WASM backend: capturing lambdas and named functions used as values now stub cleanly rather than emitting an undefined local reference.** Both produced a `local.get $name` for a local that was never declared, and an undefined local makes `wasm-tools` reject the *entire module* — so one unsupported closure took down every unrelated function in the file. Both are detected up front and stubbed, keeping the failure inside the one function that can't be compiled. (Surfaced by the stub gate above: the two excluded closure files were emitting zero stubs *and* invalid WASM.)

### Fixed — #2014 review

Seven ways the emitter could produce an **invalid module** while `mvl build --backend=wasm` exited 0, printed no warning, and `stubbed_fns()` reported zero stubs. Stubbing was made loud above; emitting unloadable WASM was the remaining silent channel, and the corpus runner could not see it either because it assembled with `wasm-tools parse`, which does not type-check.

- **The test pipeline now validates, not just assembles.** `parse` accepts a body that declares `(result i32)` and pushes nothing; `validate` rejects it. Added a `wasm-tools validate` step to the corpus runner, and `wat`/`wasmparser` dev-dependencies so `cargo test` can assemble and validate emitted modules directly — the new `validated_module_tests` module asserts on validated modules rather than on substrings of generated text, which is what let the bugs below through.
- **String literals reachable only from a generic body were interned by nobody.** `literal_scan_fns` covered the direct-emission buckets only, so `fn List[T]::tag(self) -> String { "x" }` emitted a body of just `;; missing literal` under a `(result i32 i32)` signature. Affected generic extension methods (new with the fourth bucket) and generic plain fns (pre-existing).
- **Non-generic extension-method bodies were never seeded into instantiation collection.** `fn Widget::doubled(self) { self.items.map(f) }` emitted `call $List_map__Int__Int` for an instance nobody collected. Before generic extension methods were emittable this stubbed loudly; making them emittable without seeding their callers turned a stub into a dangling symbol.
- **Instantiation collection never recursed into a lambda body** (nor into list/map/set literals, `Construct`, `Propagate`, `Relabel`, `Spawn`, `Select`). A generic call inside an HOF's lambda argument — the shape this feature exists for — was invisible to collection but still emitted. The recursion match is now exhaustive, so a new `TirExprKind` forces a decision here instead of silently falling through.
- **Lambda table slots were keyed by source span alone.** One lambda inside a generic body is compiled once per instantiation, so `T → String` was handed the slot *and the compiled body* belonging to `T → Int`, trapping on `call_indirect`'s dynamic type check. Where two concrete types lower to the same width it was worse than a trap — the wrong specialization ran silently. Keyed on the substitution too.
- **`indirect_sig` ignored the String `(ptr, len)` convention.** `wasm_ty` has no `Ty::String` arm and falls through to `_ => "i64"`; every other site tests `peels_to_string` first, this one did not. A `fn(String) -> Int` lambda got `(type (func (param i64) (result i64)))` against a two-`i32` body. WASM checks `call_indirect` types dynamically, so it passed validation and trapped at runtime.
- **`for s in strings` inside a monomorphized body referenced an undeclared local.** The ctx-blind locals pass sees the element type as `T`, so it never declared the `*MvlString` unpack temp that `emit_for_stmt` emits once `T` resolves to String. An undeclared local makes wasm-tools reject the *whole* module.
- **A generic fn calling another (or itself) mangled to `__Unknown`.** Emission inferred the substitution without resolving it through the enclosing instantiation, and both inference helpers bound a type param only when the formal was a bare `Ty::Named` — so `fn outer[T](xs: List[T])` never bound `T`. Both now use `unify_ty_params`, which this PR already added for the method path, and emission resolves through `type_subst` exactly as collection does. Pre-existing on main.

Also fixed, from the same review:

- **`.slice()` on a `List[String]` double-freed every element.** The byte-wise copy duplicates `*MvlString` pointers without bumping refcounts, while `local_drop_fn` maps both parent and slice to `_mvl_string_ptr_array_drop`; Rust's wasm allocator does not detect it, so it corrupted silently rather than trapping. New `slice_is_supported` guard stubs for pointer-shaped element types, mirroring `.clone()`'s existing precedent. Scalar slicing is unchanged. Latent before this — every existing call site slices `List[Int]`.
- **`mangle_ty_tag` collapsed `Map`/`Set`/`Result`/`Array`/`Fn`/`Unit` to the literal `"Unknown"`.** Instantiation dedup keys on the mangled name, so two different substitutions shared one emitted body and every call site got whichever was collected first — a silently wrong callee, newly reachable once `map`/`filter`/`fold` could instantiate on collection-shaped element types.
- **`is_i32` was the one type predicate without a `type_subst` arm**, so with `U → Bool` it answered `false` while `wasm_ty(U)` answered `"i32"` — the `.get` arm then read 8 bytes where `_mvl_array_push_i32` wrote 4. The `.get` arm asks `wasm_ty` directly now, agreeing with `push_op_for`/`list_elem_load_op` by construction.
- **`make wasm-stub-report` was fail-open and unwired.** It ignored `mvl build`'s exit status, so a panic whose output lacked the grep phrase reported "✓ no stubbed functions"; and it ran in no gate at all — absent from `make test`, `test-full`, `.PHONY`, and CI. Now checks the exit status, distinguishes "build failed" from "stub found", runs in `test-full`, and the warning phrase it greps for is pinned by a unit test so rewording cannot silently disable it.
- `_mvl_array_slice` returns 0 for a null receiver instead of fabricating an 8-byte-stride array, matching every other null guard in that file; added a cross-backend clamping parity test against `runtime/llvm`'s `_mvl_list_slice`, matching the three `_mvl_string_split` already had.
- `emitter_handles_method_natively` now lists `clone` and `slice`, which do have native arms; `ADR-0059`'s Consequences section no longer contradicts its own §2 scope note ("No tables or indirect calls"); the emitter's module header no longer lists higher-order fns as unsupported (only *capturing* closures are); `runtime/wasm`'s header no longer claims explicit `return` is not drop-aware. Internal: one `derived_ctx` constructor replaces two hand-copied 24-field `Ctx` literals, `emit_program`'s four overlapping bucket filters became one pass over a 2×2 match, and the `;; unsupported` marker — whose presence in a body *is* the stub trigger — is a `const` at the five scan sites.

### Fixed

- **`tests/checker_parity/baseline.tsv` was stale**, missing `tests/corpus/04_types/struct_extension_method_test.mvl` (added by #2059). The baseline is one sorted line per file, so the gap shifted every subsequent line and `checker_parity_baseline_stable` reported drift across ~40 unchanged files. Regenerated; exactly one line added, verdict `ok`, no existing verdict changed.
...
- **WASM backend: Map[String, Int] keys now stored as `*MvlString` handles.** `MvlMapEntry.key` was `Vec<u8>` — freed as raw bytes, never calling `_mvl_string_drop`, leaking the key MvlString allocation on every map scope exit. Keys are now `i32` handles; `_mvl_map_drop_si64` iterates entries and explicitly drops each key, consistent with `List[String]`/`Set[String]` (PR #1992).
