...
marimo_app = (
    ...
    .with_app(path="/epics", root=str(APP_DIR / "epic_tracker.py"))
    .with_app(path="/tests", root=str(APP_DIR / "test_results.py"))
    .with_app(path="/releases", root=str(APP_DIR / "release_timeline.py"))
    .with_app(path="/specs", root=str(APP_DIR / "spec_compliance.py"))
    .with_app(path="/metrics", root=str(APP_DIR / "codebase_metrics.py"))
).build()
...
if __name__ == "__main__":
    ...
    print(f"  http://localhost:{PORT}/epics     — Epic Tracker")
    print(f"  http://localhost:{PORT}/tests     — Test Results")
    print(f"  http://localhost:{PORT}/releases  — Release Timeline")
    print(f"  http://localhost:{PORT}/specs     — Spec Compliance")
    print(f"  http://localhost:{PORT}/metrics   — Codebase Metrics")
    print()
